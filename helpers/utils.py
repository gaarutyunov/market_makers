...
def create_plot(dt_f, title, div):
    ...
    ax.yaxis.set_ticks(np.arange(0, end_x, div if div == 1000 else div / 1000))
    start_y, end_y = ax.get_xlim()
    plt.xticks(list(range(len(indexes))), indexes)
    ax.xaxis.set_ticks(np.arange(0, end_y, 5))
    plt.yticks(fontsize=12)
    plt.ylabel('Объем, тыс шт. ценных бумаг' if div == 1000 else 'Объем, млн шт. ценных бумаг', fontsize=12)
    ax.set_title(title)
